// ===================== Flower Blossom: loading-screen animation =====================
// Plays after the bunny-hug game finishes, then hands off to the next game.
// Animation logic ported as-is from the provided flower-blossom.html/GSAP source,
// just scoped so it only runs once startFlowerBlossom() is called.

(function () {
  const DISPLAY_DURATION = 7000; // ms the flower animation plays before moving on
  let started = false;
  ...
  // Called by bunny-game.js once its own win message has been shown
  window.startFlowerBlossom = function () {
    if (started) return;
    started = true;

    $('#flowerBlossomOverlay').show();
    playAnimation();
